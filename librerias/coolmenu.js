 ...
 * Diciembre, 2004.
 *
 * Copyright (C) 2003 por Thomas Brattli - Héctor Díaz Díaz - Patricio Merino Díaz.
 ...
 *
 * Rutinas para la creación de un menú desplegable totalmente configurable. El menú tiene
 * movimiento e interacción con el usuario.
 */
...
function cm_checkScrolled(obj)
{
	...
	if(obj.scrolledY!=obj.lastScrolled)
	{
		if(!obj.useframes)
		{
			for(i=0;i<obj.l[0].num;i++)
			{
				var sobj=obj.l[0].o[i].oBorder;
				sobj.moveY(sobj.y+(obj.scrolledY-obj.lastScrolled));
			}
			if(obj.usebar) obj.oBar.moveY(obj.oBar.y+(obj.scrolledY-obj.lastScrolled));
		}
		obj.lastScrolled=obj.scrolledY;
		page.y=obj.scrolledY;
		page.y2=page.y2orig+obj.scrolledY;
...
function cm_makeObj(obj,name,level,win,nest,o)
{
	...
	this.ref=bw.dom?document:this.evnt.document;
	this.css=bw.ns4?this.evnt:this.evnt.style;
...
{
	if(!this.vis) return;
	if(this.clipy<ystop-px)
	{
		this.clipy+=px;
		this.clipTo(0,w,this.clipy,0,1);
		this.tim=setTimeout(this.obj+".clipOut("+px+","+w+","+ystop+","+tim+",'"+name+"')",tim);
	}
	else
	{
		if(bw.ns6) this.hideIt();
		this.clipTo(0,w,ystop,0,1);
		if(bw.ns6) this.showIt();
	}
}
function cm_slide(end,px,tim,w,h)
{
	...
	if(this.y<end-px)
	{
		if(this.y>(end-px*px-px)&&px>1) px-=px/5;
		this.moveIt(this.x,this.y+px);
...
function cm_setactive(on,name,frmmouse)
{
	...
	if(tobj.img)
	{
		if(tobj.img2)
		{
			if(on) this.ref.images[tobj.img].src=tobj.img2;
			else this.ref.images[tobj.img].src=tobj.img1;
		}
	}
	else
	{
		...
		{
			var color=tobj.c1;var fcolor=tobj.c3;
			var re=tobj.c4;
		}
		if(color)
		{
			if(bw.dom||bw.ie4) this.css.backgroundColor=color;
			else if(bw.ns4) if(color=="transparent") color=null;
...
function cm_makeMenu(name,parent,text,link,target,width,height,img1,img2,bgcoloroff,bgcoloron,textcolor,hovercolor,onclick,onmouseover,onmouseout)
{
	...
	if (this.l.length<=l)
	{
		...
		this.l[l].maxnum=0;this.l[l].bordercolor=prop1["bordercolor"]||prop2["bordercolor"];
	...
	if(parent!=""&&parent) obj.num=this.m[parent].subs.length-1;
...
function cm_mover(name,cl)
{
	...
	if(this.m[name].mover!="") self.status=this.m[name].mover;
...
function cm_getCoords(name,l,num,topalign,align,ln,border,cn,lev1b)
{
	...
	lx=x+pobj.width;
	rx=x-this.m[name].maxwidth-(this.l[l-1].offsetX*2);
	if(align==0)
	{
		if(l==1) y+=border;
		if(l==1&&this.useframes) x=0;
		else x=lx;
	}
	else if(align==1)
			 {
				...
				if(l==1)
				{
					if(this.useframes) x=page.x2-this.m[name].maxwidth;
					y+=border;
				}
			 }
	if((align==2||topalign==2)&&lev1b!=3)
	{
		if(l!=1&&(align!=1&&align!=0))
		{
			if(topalign==1) x=rx;
			else x=lx;
		}
		if(l==1)
			if(this.useframes) y=0;
			else y+=this.m[name].height+border+pborder;
	}
	...
	this.m[name].subx=x+this.l[l-1].offsetX;
	this.m[name].suby=y+this.l[l-1].offsetY;
...
function cm_checkPage(name,l,num,topalign,align,ln,border,cn)
{
	...
	if(this.m[name].subx+this.m[name].maxwidth>page.x2)
	{
		if(align!=1)
		{
			...
			align=1;
			this.getCoords(name,l,num,topalign,align,ln,border,cn);
		}
	}
				...
				if(align!=0)
				{
					...
					this.getCoords(name,l,num,topalign,align,ln,border,cn);
			...
			else if((this.m[name].suby+this.m[name].totheight)>page.y2)
						 {
							...
							{
								topalign=3;
								this.getCoords(name,l,num,topalign,align,ln,border,cn,3);
							}
							else if(align!=3)
									 {
									 	align=3;
							...
							else if(this.m[name].suby<page.y)
									 {
										...
										{
											topalign=2;
											this.getCoords(name,l,num,topalign,align,ln,border,cn,2);
										}
										else if(align!=2)
										{
											align=2;
...
function cm_showSubs(name,l,num,cl)
{
	...
	for(i=0;i<this.l[l].maxnum;i++)
	{
		...
		if(i<ln)
		{
			...
			obj.aname=n;
			if(!bw.ns4||!this.NS4hover) obj.writeIt(this.m[n].text)
	...
	if(this.l[l-1].filter&&bw.filter) bobj.filterIt(this.l[l-1].filter);
			 ...
			 {
			 	bobj.showIt();
				bobj.clipOut(this.l[l-1].clippx,this.m[name].maxwidth,this.m[name].totheight,this.l[l-1].cliptim,name);
			 }
			 else if(this.l[l-1].slide)
			 			{
							bobj.showIt();
							bobj.slide(y,15,30,this.m[name].maxwidth,this.m[name].totheight);
						}
						else bobj.showIt();
...
function cm_makeTop(rr)
{
	...
	if(this.usebar)
	{
		...
		if(this.barwidth=="menu")
		{
			...
			if(this.usetextwidth) this.maxwidth=0;
			for(j=0;j<this.l[0].maxnum;j++)
			{
				...
				if(this.usetextwidth==1) w+=obj.w;
		...
		{
			if(bw.ns4||bw.ns6)
				if(this.checkscroll!=2&&this.useframes!=1) setTimeout("cm_checkScrolled("+this.name+")",200);
		}
...
function cm_refresh(ev)
{
	...
	if(this.useframes) page=new makePageCoords(this.win,this.useframes);
	else page=toppage;
	for(i=1;i<this.l.length;i++)
	{
		...
		if(bw.ns4)
		{
			...
			oNS.zIndex=(500+i);
			if(this.l[i].border) oNS.bgColor=this.l[i].bordercolor;
		}
		else if(bw.ns6)
				 {
		...
		for(j=0;j<this.l[i].maxnum;j++)
		{
			if(bw.ns4) oNS2=new Layer(this.l[i].width,oNS);
					 ...
					 {
					 	oNS2=document.createElement("DIV");
						oNS2.setAttribute("style",this.ns6style[i]);
						oNS.appendChild(oNS2);
					 }
	...
	this.loaded=1;
	if(this.checkscroll&&bw.ie)
	{
		this.win.document.body.onscroll=new Function("cm_checkScrolled("+this.name+")");
		cm_checkScrolled(this);
	}
	if(this.useclick)
	{
		this.win.document.onmouseup=new Function(coolFMouseup);
		if(this.useframes) document.onmouseup=new Function(coolFMouseup);
	}
...
function cm_makeStyle()
{
    ...
    if(!this.barbackgroundimg)
		...
		else
		{
			...
			st+=bw.ns4?"layer-background-image":"background-image";
			st+=":url('"+this.barbackgroundimg+"');";
		}
...
function cm_resized()
{
	...
	if(page2.x2!=toppage.x2||page2.y2!=toppage.y2)
	{
		if(!bw.ns4)
		{
			...
			this.makeTop(1);
			if(!this.useframes) page=toppage;
			this.isresized=1;
...
function cm_go(name)
{
	...
	if(url)
	{
		if(this.useframes&&!coolFrameError) loc=this.win.location.href;
		else loc=location.href;
		if(fc) eval(fc);
		url=this.checkFolder(loc.toString(),url);
		this.isover=0;
		this.hideSubs(1,0,1);
		this.isclicked=0;
		this.aobj[0]=-1;
...
function cm_checkLoaded(ev,ns)
{
	...
	if(document.layers)
	{
		if(ns)
		{
			...
			if(ev.target.name==coolFName.frame&&!coolFName.nsload)
			{
				coolFName.refresh();
				ok=1;
				coolFrameError=0;
		...
		if(bw.ie)
		{
			if(coolFName.win.document.readyState=="complete")
			{
				...
				coolFName.win.document.body.onunload=cm_unloaded;
				coolFName.refresh();
				ok=1;
				coolFrameError=0;
			}
			...
			if(bw.ns6)
			{
				...
				{
					if(coolFName.win.document.body)
					{
						coolFName.win.addEventListener("unload",cm_unloaded,true);
						coolFName.refresh();
						ok=1;
						coolFrameError=0;
					}
...
function cm_checkFrame(num)
{
	...
	if(num==10)
	{
		debug('Frame: '+this.frame+' doesn\'t exist - Value: '+fr+" - Could not build menus.");
		return;
	}
	if(!top.frames[this.frame]) fr=findFrame(this.frame);
	else fr=top.frames[this.frame];
	if(!fr)
	{
		num++;
	...
	else
	{
		...
		if(bw.ns4)
		{
			...
			top.frames.onunload=cm_unloaded;
			top.frames.onload=cm_checkLoaded;
			setTimeout("cm_checkLoaded(0,1)",1000);
		}
