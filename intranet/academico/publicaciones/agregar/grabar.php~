...
<HTML>
<HEAD>
<TITLE>INTR@NET Escuela Ingenier&iacute;a en Computaci&oacute;n - Acad&eacute;mico - Agregar Publicaci&oacute;n</TITLE>
...
if (isset($_POST['titulo']) && isset($_POST['tipo_publicacion']) && isset($_POST['anio']) && isset($_POST['en_biblioteca']) && is_string($_POST['titulo']) && is_numeric($_POST['tipo_publicacion']) && is_numeric($_POST['anio']) && is_numeric($_POST['en_biblioteca']))
	$correcto = true;
...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0">
  ...
  <TR>
    ...
    <TD WIDTH="500" HEIGHT="413" VALIGN="TOP"> 
      <?PHP
			/*
			 * Script en donde se crea un objeto de publicación y se graba la nueva
			 * publicación en la base de datos.
			*/
			
			// Cuando los parámetros son correctos.
			if ($correcto)
			{
				...
				// Abrimos la sesión, creamos un objeto publicacion y agregamos la publicacion
				// a la base de datos.
				$publicacion = new publicacion($link);
				
				// Cuando la publicación no es tesis.
				if ($_POST['tipo_publicacion'] != 2)
					$publicacion->agregar($_POST['titulo'], $_SESSION["id_persona"], $_POST['tipo_publicacion'], $_POST['empresa'], $_POST['anio'], $_POST['en_biblioteca']);
				// Cuando la publicación es tesis.
				else $publicacion->agregar($_POST['titulo'], $_SESSION["id_persona"], $_POST['tipo_publicacion'], -1, $_POST['anio'], $_POST['en_biblioteca']);
				
				// Desconectamos la base de datos.
				$conexion->desconectar();
			}
			// Cuando los parámetros no son correctos.
			else
			{
				// Librerías necesarias.
